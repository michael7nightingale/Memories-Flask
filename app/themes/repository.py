from typing import Any

from app.db.repositories.base import BaseRepository
from .models import Theme, Card
from ..extensions import db


class ThemeRepository(BaseRepository):
    def __init__(self):
        super().__init__(model=Theme)

    def all_by_user(self, user_id: int) -> list[Any]:
        return super().filter_by(user_id=user_id)

    def find(self, string: str) -> list[Any]:
        return self._model.query.filter_by(isPublic=True).filter(self._model.title.contains(string)).all()


class CardRepository(BaseRepository):
    def __init__(self):
        super().__init__(model=Card)

    def all_by_theme(self, theme_id: int) -> list[Any]:
        return super().filter_by(theme_id=theme_id)

    def count_by_theme(self, theme_id: int) -> int:
        len_cards = self._model.query.filter_by(theme_id=theme_id).count()
        return len_cards

    def delete_by_theme(self, theme_id: int) -> None:
        cards = super().filter_by(theme_id=theme_id)
        db.session.delete(cards)
        db.session.commit()
